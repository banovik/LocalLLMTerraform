...
resource "docker_container" "open_webui" {
  ...
  # WSL2 or native Docker Desktop with GPU support required
  runtime = "nvidia"
  env = [
...
# Get UserID and GroupID (1000 is default for most Linux environments)
data "external" "user_info" {
  program = ["PowerShell", "-Command", "Write-Output (ConvertTo-Json @{user_id='1000'; group_id='1000'})"]
}